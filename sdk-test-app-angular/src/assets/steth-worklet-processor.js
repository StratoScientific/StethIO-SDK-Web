...
class StethWorkletProcessor extends AudioWorkletProcessor {
    ...
    handleMessage_(event) {
        ...
        if (event.data.type == 'samples') {
          ...
          this.changePlaybackSpeed(event.data.speed);
        } else if(event.data.type === 'volume'){
            console.log(event.data)
            this._outputRingBuffer.push([event.data.samples], event.data.heartMode, event.data.gain, true);
        } else {
...
class RingBuffer {
    ...
    gainFilter(arraySequence, heartMode , gain) {
